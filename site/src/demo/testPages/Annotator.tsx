...
import { Layout, Flex, Button, Typography, ConfigProvider, Card, Table, Pagination, Input, Space, Tag, Select, message, Progress } from "antd";
...
import testData from '../../../static/test/pipeline/annotator.json';
  ...
  useEffect(() => {
    ...
    const transformed = testData.map((item: any, index) => {
      ...
      return {
        ...
        check:  
          (testRes && expectedRes && 
           JSON.stringify(testRes) === JSON.stringify(expectedRes))
      } as AnnotatorData;
    });
  ...
  const updateExpectedRes = (id: string, newValue: string) => {
    const updatedData = processedData.map(item => {
      if (item.id === id) {
        ...
        // 重新计算检查结果
        updated.check = (updated.testRes && updated.expectedRes && 
        ...
        setTimeout(async () => {
          ...
          if(test){
            ...
            newData[i] = {
              ...item,
              testRes: testRes,
              check: (testRes && item.expectedRes && 
                JSON.stringify(testRes) === JSON.stringify(item.expectedRes))
            };
          }else{
    ...
    },
    {
      ...
      render: (type: string, record: AnnotatorData) => (
        <Select
          style={{ width: '100%' }}
          value={type}
          onChange={(value) => updateExpectedRes(record.id!, value)}
        >
        ...
        <Card title='控制面板'>
          <Space direction="vertical" style={{ width: '100%' }}>
            <Space>
              ...
              <Button 
                type="default" 
                onClick={()=>{stoppingRef.current = true}}
                danger
                disabled={!loading}
              >
              ...
              <Button 
                type="default" 
                onClick={()=>{runAllTests(finishCount.current)}}
              ...
              <Button 
                type="default" 
                onClick={()=>{setFinishCount(0); setProgress(0)}}
                disabled={loading || finishCount.current == 0}
              >
                重置
              </Button>
          ...
          extra={
            finishCount.current > 0 ? (
              <Space>
                ...
                <Typography.Text>
                  正确: <Tag color="green">{processedData.slice(0, finishCount.current).filter(item => item.check === true).length}</Tag>
                </Typography.Text>
                <Typography.Text>
                  错误: <Tag color="red">{processedData.slice(0, finishCount.current).filter(item => item.check === false).length}</Tag>
                </Typography.Text>
                <Typography.Text>
